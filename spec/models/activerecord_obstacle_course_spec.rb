...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:order_items).where("orders.user_id = ? AND order_items.item_id = ?", 2, 4)
    # -----------------------------------------------------------
  ...
  it 'returns the names of items that are associated with one or more orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    # Solution goes here
    Item.joins(:order_items).distinct.pluck(:name)
  ...
  it 'returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).select("users.name, count(item_id) AS total_item_count").group(:name).order(name: :desc)
    # ---------------------------------------------------------------
